...
Each bubble is four concentric discs — a flat white texture tinted green,
rounded off by Blizzard's portrait mask, blended additively so the layers sum to
a soft core. Two things there are load-bearing: the mask must be set before the
texture, and the texture must be a real file, because a mask over a
`SetColorTexture` fill has no effect and draws squares. Size, count, speed,
opacity and the layer falloff are the `BUBBLE_*` constants at the top of
`UI.lua`.
...
Released on CurseForge as
[Mysterious Mix Helper](https://www.curseforge.com/wow/addons/mysterious-mix-helper).

The wiring is already done: the project id is in the TOC as
`## X-Curse-Project-ID`, an author API token sits in the `CF_API_KEY` GitHub
secret, and the project's Source tab is deliberately left on "No automatic
packaging" so CurseForge does not build a competing zip. Pushing a `v*` tag
uploads there and creates the GitHub release in one go.

The page's own text — summary, description in English and French, categories —
is kept in [docs/curseforge.md](docs/curseforge.md) so it can be edited in the
repository rather than only in a web form. The avatar is
[docs/avatar-400.png](docs/avatar-400.png), regenerated by
`tools/New-Avatar.ps1`. [screenshots/](screenshots/README.md) carries the shot
list for the page's images.
